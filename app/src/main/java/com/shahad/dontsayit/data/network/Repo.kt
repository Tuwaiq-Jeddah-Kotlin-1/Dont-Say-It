...
class Repo(val context: Context) {
    ...
    suspend fun createUser(username: String, email: String, pass: String,findNavController:NavController) =
        withContext(Dispatchers.IO) {
            auth.createUserWithEmailAndPassword(email, pass).addOnCompleteListener { task ->

                if (task.isSuccessful) {
                    savePersist(username, email)
                    storeUserData(User(username, email),findNavController)
    ...
    private fun savePersist(username: String, email: String) {
    ...
    suspend fun updateUser(username: String) = withContext(Dispatchers.IO) {
        db.collection(collection).document(auth.currentUser!!.uid).update("username",username).addOnCompleteListener {
    ...
    suspend fun signIn(email: String, pass: String,findNavController:NavController) = withContext(Dispatchers.IO) {
        auth.signInWithEmailAndPassword(email, pass).addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                savePersist("signIn default",email)
